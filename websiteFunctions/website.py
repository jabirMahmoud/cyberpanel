...
""" % (self.domain, website.externalApp)
            ...
            command = "cp /home/cyberpanel/config /home/%s/.ssh/config" % (self.domain)
            ProcessUtilities.executioner(command, website.externalApp)

            command = "chown %s:%s /home/%s/.ssh/config" % (website.externalApp, website.externalApp, self.domain)
            ProcessUtilities.executioner(command)

            command = "chmod 600 /home/%s/.ssh/config" % (self.domain)
            ProcessUtilities.executioner(command, website.externalApp)

            command = "rm -rf /home/cyberpanel/config"
            ProcessUtilities.executioner(command)

            command = "cp /home/%s/.ssh/id_rsa.pub /home/cyberpanel/%s.pub" % (self.domain, self.domain)
            ProcessUtilities.executioner(command)

            command = "chown cyberpanel:cyberpanel /home/cyberpanel/%s.pub" % (self.domain)
            ProcessUtilities.executioner(command)
            ...
            proc = httpProc(request, 'websiteFunctions/setupGit.html',
                            {'domainName': self.domain, 'installed': 0, 'webhookURL': webhookURL})
            return proc.render()
    ...
    def startCloning(self, userID=None, data=None):
        try:
            ...
            if ACLManager.checkOwnership(self.domain, admin, currentACL) == 1:
                ...
                return ACLManager.loadErrorJson('cloneStatus', 0)

            background = Cloner(self.domain, repoURL, branch, repoName, tempStatusPath)
            background.start()

            time.sleep(2)

            data_ret = {'status': 1, 'cloneStatus': 1, 'error_message': 'None',
                        'tempStatusPath': tempStatusPath}
            json_data = json.dumps(data_ret)
            return HttpResponse(json_data)

        except BaseException as msg:
            data_ret = {'status': 0, 'cloneStatus': 0, 'error_message': str(msg)}
            json_data = json.dumps(data_ret)
            return HttpResponse(json_data)

    def cloneStatus(self, userID=None, data=None):
        try:
            statusFile = data['statusFile']
            ...
            if ACLManager.CheckStatusFilleLoc(statusFile):
                ...
                data_ret = {'abort': 1, 'cloneStatus': 0, 'installationProgress': "100",
                            'currentStatus': 'Invalid status file.'}
                json_data = json.dumps(data_ret)
                return HttpResponse(json_data)
            ...
            if lastLine.find('[200]') > -1:
                command = 'rm -f ' + statusFile
                subprocess.call(shlex.split(command))
                data_ret = {'abort': 1, 'cloneStatus': 1, 'installationProgress': "100",
                            'currentStatus': 'Successfully Installed.'}
                json_data = json.dumps(data_ret)
                return HttpResponse(json_data)
            elif lastLine.find('[404]') > -1:
                data_ret = {'abort': 1, 'cloneStatus': 0, 'installationProgress': "0",
                            'error_message': ProcessUtilities.outputExecutioner("cat " + statusFile).splitlines()}
                json_data = json.dumps(data_ret)
                return HttpResponse(json_data)
            else:
                ...
                try:
                    ...
                    installationProgress = 0
                data_ret = {'abort': 0, 'cloneStatus': 0, 'installationProgress': installationProgress,
                            'currentStatus': currentStatus}
                json_data = json.dumps(data_ret)
                return HttpResponse(json_data)

        except BaseException as msg:
            data_ret = {'abort': 0, 'cloneStatus': 0, 'installationProgress': "0", 'error_message': str(msg)}
            json_data = json.dumps(data_ret)
            return HttpResponse(json_data)
    ...
    def gitPull(self, userID=None, data=None):
        try:
            ...
            background = GitPuller(self.domain, repoURL, branch, repoName, tempStatusPath)
            background.start()
    ...
    def gitPullStatus(self, userID=None, data=None):
        try:
            statusFile = data['statusFile']
    ...
    def getSSHConfigs(self, userID=None, data=None):
        try:
            ...
            if ACLManager.checkOwnership(self.domain, admin, currentACL) == 1:
                ...
                return ACLManager.loadErrorJson('getSSHConfigs', 0)

            website = Websites.objects.get(domain=self.domain)

            path = "/home/" + self.domain + "/.ssh/config"
            ...
            data_ret = {'getSSHConfigs': 1, 'error_message': "None", "configData": configData}
            json_data = json.dumps(data_ret)
            return HttpResponse(json_data)

        except BaseException as msg:
            data_ret = {'getSSHConfigs': 0, 'error_message': str(msg)}
            json_data = json.dumps(data_ret)
            return HttpResponse(json_data)
    ...
    def saveSSHConfigs(self, userID=None, data=None):
        try:
            ...
            if ACLManager.checkOwnership(self.domain, admin, currentACL) == 1:
                ...
                return ACLManager.loadErrorJson('saveSSHConfigs', 0)
            ...
            command = "chown %s:%s %s" % (website.externalApp, website.externalApp, path)
            ProcessUtilities.executioner(command)

            command = "chmod 600 %s" % (path)
            ProcessUtilities.executioner(command)

            data_ret = {'saveSSHConfigs': 1, 'error_message': "None"}
            json_data = json.dumps(data_ret)
            return HttpResponse(json_data)

        except BaseException as msg:
            data_ret = {'saveSSHConfigs': 0, 'error_message': str(msg)}
            json_data = json.dumps(data_ret)
            return HttpResponse(json_data)
...
class WebsiteManager:
    ...
    def __init__(self, data=None):
        ...
        if 'phpVersion' in data.keys():
            self.phpVersion = data['phpVersion']
        else:
        ...
        if 'websiteOwner' in data.keys():
            self.websiteOwner = data['websiteOwner']
        else:
            self.websiteOwner = None

        if 'externalApp' in data.keys():
            self.externalApp = data['externalApp']
        else:
        ...
        if 'dkimCheck' in data.keys():
            self.dkimCheck = data['dkimCheck']
        else:
            self.dkimCheck = None

        if 'openBasedir' in data.keys():
            self.openBasedir = data['openBasedir']
        else:
            self.openBasedir = None

        if 'sslForced' in data.keys():
            self.sslForced = data['sslForced']
        else:
            self.sslForced = None

        if 'http2' in data.keys():
            self.http2 = data['http2']
        else:
            self.http2 = None
        ...
        if 'wafRules' in data.keys():
            self.wafRules = data['wafRules']
        else:
            self.wafRules = None

        if 'wafMode' in data.keys():
            self.wafMode = data['wafMode']
        else:
            self.wafMode = None

        if 'wafExclusions' in data.keys():
            self.wafExclusions = data['wafExclusions']
        else:
            self.wafExclusions = None
